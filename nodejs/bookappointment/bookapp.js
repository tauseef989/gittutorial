...
app.put('/appointments/:id', async (req, res) => {
  ...
  try {
    // Check if date is empty, if so, set it to NULL
    const formattedDate = date ? date : null;

    // Update appointment in the database
    await pool.execute('UPDATE appointments SET name = ?, email = ?, phone = ?, date = ?, time = ? WHERE id = ?', [name, email, phone, formattedDate, time, id]);
