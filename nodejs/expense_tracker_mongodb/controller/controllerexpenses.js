...
exports.post = async (req, res) => {
  ...
  try {
    ...
    const newExpense = { amount, description, category, userid: new ObjectId(userid), created_at: currentTime };
...
exports.get = async (req, res) => {
  try {
    ...
    const expenses = await db.collection('expenses').find({ userid: new ObjectId(userid) }).toArray();
    const limitedExpenses = expenses.slice(start_index, start_index + parseInt(noe));
    const limit = Math.ceil(expenses.length / parseInt(noe));

    res.json({ expenses: limitedExpenses, limit });
  } catch (error) {
    console.error('Error fetching expenses:', error);
    res.status(500).json({ error: 'Failed to fetch expenses' });
  }
...
exports.delete = async (req, res) => {
  ...
  try {
    ...
    await db.collection('users').updateOne({ _id: new ObjectId(userid) }, { $inc: { total_expenses: -expense.amount } });
